...
// nvcc. The movement points match AstrocytePopulation's CUDA backend: state
...
// written for every cell (see src/cuda_kernels.cu), not just on a spike, so the
...
// (also in src/cuda_kernels.cu) read the spike flags and write the synaptic drive
