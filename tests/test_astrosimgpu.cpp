...
// shared accumulator, so those two are bit-identical for any thread count on
// their own. deliver_spikes and deliver_sic scatter-add into shared
// ring-buffer slots, where different sources can share a target -- an atomic
// there is correct but not deterministic, since floating-point addition is
// not associative and the accumulation order would depend on real-time
// thread scheduling. Both instead stage each thread's contributions
// separately and merge them in a fixed, thread-index order afterward (see
// the comment on Network::deliver_spikes), which reconstructs the same term
// order a serial loop would use regardless of thread count. So the whole
// step is bit-identical for any thread count, and the check below is exact
// equality, not a tolerance. omp_set_num_threads affects every #pragma omp
// region encountered afterward, including neurons_.update()'s own parallel
// loop, so this incidentally covers that too, not just deliver_spikes.
